...
<?php
  ...
  if (!empty($_POST)){
    ...
    $mail = $_POST['comment_mail'];
    $body = $_POST['comment_body'];

    $query = "INSERT INTO `comments` (`p_id`, `comment_user`, `comment_useremail`, `comment_body`) VALUES ({$p_id}, '{$user}', '{$mail}', '{$body}')";
    
    mysql_select_db($database_name, $consult);
    $result = mysql_query($query, $consult) or die(mysql_error());
  }

  $query1 = "SELECT * FROM comments where p_id={$p_id}";
  mysql_select_db($database_name, $consult);
  $result1 = mysql_query($query1, $consult) or die(mysql_error());
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                          ...
                          <div class="col-md-9">
                                ...
                                <h5><span class="glyphicon glyphicon-time"></span> Post by <?php echo $row['p_user']; ?>, <?php echo $row['p_date']; ?>.</h5>
                                
                                <p><?php echo $row['p_body']; ?>.</p>
                                <br><br>
                                
                                <hr>
                          
                                <h4>Leave a Comment:</h4>
                                <form role="form" name="comment_form" action="" method="POST">
                                  <div class="form-group">
                                    <input type="text" name="comment_user" class="form-control" placeholder ="Display Name" required>
                                  </div>
                                  <div class="form-group">
                                    <input type="email" name="comment_mail" class="form-control" placeholder ="Email Address" required>
                                  </div>
                                  <div class="form-group">
                                ...
                                <p><span class="badge"><?php echo $rows; ?></span> Comments:</p><br>
                                
                                <div class="row">
                                  <?php if($rows<1){
                                    echo "<h3 class='mx-auto'>Be the first to comment</h3>";
                                  }else{ do{ ?>
                                  <div class="col-sm-2 text-center">
                                    <img src="../media/avatar.jpg" class="img-circle" height="65" width="65" alt="Avatar">
                                  </div>
                                  <div class="col-sm-10">
                                    <h4><?php echo $row1['comment_user']; ?> <small><?php 
                                    $date = $row1['comment_date'];
                                    echo date('F j, Y, g:i a',strtotime($date)); ?></small></h4>
                                    <p><?php echo $row1['comment_body']; ?></p>
                                    <br>
                                  </div>
